...
function ProgramMap(cells, defs) {
    ...
    this.canSpawn = true;
    this.spawnState = 1;
    if (this.spawnState > this.cells[0].length) this.spawnState = 0;//this.canSpawn = false;
...
ProgramMap.prototype.tick = function() {
    ...
    if (this.canSpawn) {
        ...
        this.spawnState++;
        if (this.spawnState >= this.definitions.length) this.spawnState = 0;//this.canSpawn = false;
    }
};
...
ProgramMap.fromString = function(program) {
    // allow using commas or newlines
